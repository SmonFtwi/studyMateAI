...
const UploadModal: React.FC<UploadModalProps> = ({ onClose, onUpload }) => {
  ...
  const handleFileChange = (event: React.FormEvent<HTMLInputElement>) => {
    ...
    if (input.files && input.files.length > 0) {
      ...
      if (validFiles.length !== files.length) {
        setErrorMessage(
          "Some files are not valid. Only PDF, TXT, DOC, XLS, XLSX, and CSV files are allowed.",
        );
      }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response: { results?: UploadResult[] } = await uploadPDF(
        token,
        formData,
      );
      ...
      //   setErrorMessage(`Successfully uploaded: ${successfulFiles.map((f) => f.filename).join(", ")}`);
      // }
    ...
    <Dialog open={true} onOpenChange={() => !isUploading && onClose()}>
      <DialogContent className="max-h-[70vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Choose documents to add to your file library.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="mt-4">
          <Button variant="ghost" disabled={isUploading} onClick={onClose}>
            Cancel
          </Button>
          <Button
