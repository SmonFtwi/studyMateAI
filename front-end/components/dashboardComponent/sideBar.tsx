...
import { GraduationCap, LogOut, MoreVertical, Settings, Sparkles, Trash2, User } from 'lucide-react';
...
export default function Sidebar() {
  ...
  const fetchChatSessions = async (token:string) => {
    try {
      const response = await listChatHistory(token);
  ...
  const handleDeleteChat = async (session_id:any) => {
    const token = localStorage.getItem("token") as string;
    const response = await deleteChat(session_id, token);
    ...
    <div className="flex flex-col h-full relative rounded-lg text-gray-800 dark:text-white overflow-hidden border border-blue-200/50 dark:border-white/10">
          ...
          {chatSessions.length > 0 ? (
            chatSessions.map((chat) => (
              ...
              >
                {/* Chat Title */}
                <Link
                  href={`/Dashboard/?session_id=${chat.session_id}`}
                ...
                >
                  ...
                  <span className="truncate text-gray-700 dark:text-white/90 hover:text-gray-900 dark:hover:text-white transition-colors">
                    {chat.session_title.split(' ').slice(0, 3).join(' ')}
                  </span>
                </Link>
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-600 dark:text-white/60">No chats available yet</p>
              <p className="text-xs text-gray-500 dark:text-white/40 mt-1">Start a new conversation to get started</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="p-4 space-y-3 relative z-10">
            ...
            <TooltipProvider>
              <div className="grid grid-cols-3 gap-2">
                <Tooltip>
                  ...
                  <TooltipContent side="top" className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-md border-blue-200/50 dark:border-white/10">
        ...
        <DropdownMenu>
          ...
          >
            <DropdownMenuLabel className="text-gray-700 dark:text-white/80">Account</DropdownMenuLabel>
